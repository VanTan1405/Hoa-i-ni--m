...
export async function apiFetch(path:string,options:RequestInit={}){if(staticMode()&&!window.A14_CONFIG?.apiBase)throw new Error("Bản GitHub Pages chưa được kết nối máy chủ. Hãy cấu hình apiBase trong a14-config.js để bật đăng nhập và lưu Google Drive.");const token=typeof sessionStorage!=="undefined"?sessionStorage.getItem("a14_session"):null;return fetch(apiUrl(path),{...options,credentials:"include",headers:{...(token?{Authorization:"Bearer "+token}:{}),...options.headers}})}
